...
在 `CL1_Core/` 里的改动需要分两步提交：

```bash
# 在 submodule 提交并推送到 0611roxe/CL1_Core 的 csr 分支
cd CL1_Core
git add -A && git commit -m "feat(...): ..."
git push

# 在主仓库里更新 submodule 指针
cd ..
git add CL1_Core
git commit -m "chore: bump CL1_Core submodule (<brief comment>)"
git push
```

拉取别人更新：`git pull && git submodule update --init --recursive`。
